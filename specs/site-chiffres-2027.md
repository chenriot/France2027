...
| **CSS natif** (custom properties) | Le CSS existant est déjà bon et tient en 292 lignes. Il est repris **verbatim**, avec ses noms de classes, pour tout ce que rend `/tout` : `check:render` y compare les attributs, donc une classe ajoutée à un élément de chapitre casse la vérification. **Hors de ce périmètre — routes nouvelles, sommaire, rail, ajouts en `mode="page"` — Tailwind, les CSS Modules et le CSS-in-JS sont autorisés.** Seule contrainte partout : les couleurs viennent des jetons de `tokens.css`, sinon la bascule clair/sombre tombe. |
